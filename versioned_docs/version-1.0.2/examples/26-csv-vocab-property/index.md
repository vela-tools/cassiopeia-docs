...
sidebar_label: "VocabProperty"
sidebar_position: 26
description: "Turn an OpenStreetMap tag into a VocabProperty IRI."
keywords: ["NGSI-LD", "worked example", "csv to NGSI-LD", "UrbanMobilityPoint"]
---
...
This is the third custom-model example. The [data-model guide](../../guides/data-models.md) explains why a controlled vocabulary can belong in a custom model.

## What it teaches

- The `VocabProperty` type and its `vocab` IRI.
- How to distinguish a shared-vocabulary term from an opaque code (`Property`) or a link to an entity ([`Relationship`](../10-json-relationships/index.md)).
...
`amenity` comes from OpenStreetMap's fixed [`amenity` key vocabulary](https://wiki.openstreetmap.org/wiki/Key:amenity), while `name` is often empty free text. The `@type`, `@id`, `@lat`, and `@lon` headers are not valid template identifiers, so the mapping uses the `this['...']` bracket form from the [messy-headers example](../06-csv-messy-headers/index.md). `@type` can be `node`, `way`, or `relation`, and an OSM ID is unique only within one type, so the identity combines both fields.
...
The complete mapping is in [amenity.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/26-csv-vocab-property/amenity.json5):
