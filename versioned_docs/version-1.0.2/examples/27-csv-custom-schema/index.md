...
sidebar_label: "Custom schemas"
sidebar_position: 27
description: "Validate two invented models against two local JSON Schemas in one run."
keywords: ["NGSI-LD", "worked example", "csv to NGSI-LD", "ExoPlanet", "Star"]
---
...
[Example 20](../20-csv-at-context/index.md) created an `ExoPlanet` model and local `@context`, but did not validate it because the Smart Data Models catalog has no schema for an invented model. This example supplies two local schemas and validates both models in one run: `ExoPlanet` from the planet dataset and `Star` from a separate host-star dataset.
...
Unlike [example 20](../20-csv-at-context/index.md#the-synthetic-host-star), which creates the star with a `syntheticEntity`, this run reads stars from their own dataset. The relationship supplies only the target URN; the second input creates the `Star` entity with that ID. Both mappings use `hostname` for the ID, so `HD 330075` becomes `urn:ngsi-ld:Star:HD330075` on both sides. The mappings are in [planet.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/27-csv-custom-schema/planet.json5) and [star.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/27-csv-custom-schema/star.json5).
...
[exoplanet.schema.json](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/27-csv-custom-schema/exoplanet.schema.json) constrains the planet:
...
[star.schema.json](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/27-csv-custom-schema/star.schema.json) constrains the star, with the ranges each quantity actually occupies, a positive temperature, a right ascension in `[0, 360]`, a declination in `[-90, 90]`, a positive distance:
...
The per-input `schema` takes precedence for its own type, and the global source supplies the rest. This allows one model to use a hand-authored schema while other types use the catalog convention or a shared fallback. `--validation-schema` is the single-input form of the same setting. A remote HTTPS schema is downloaded once at the start; a failed fetch or missing schema aborts the run. Schemas belong to the run, never to a mapping document. The [validation guide](../../guides/validation.md#custom-schemas) covers precedence and local-versus-URL rules.

## Related pages

- [Validation](../../guides/validation.md), the modes, the representation, and the custom-schema entry points in reference form.
- [Advanced JSON schema validation for normalized output](../28-json-advanced-schema/index.md), a custom schema that constrains the full normalized wrapper, with `unitCode`, `observedAt`, `datasetId`, and the LanguageProperty, VocabProperty, and ListProperty kinds.
- [A custom model with its own `@context`](../20-csv-at-context/index.md), the same planet model, with a synthetic host star and a local `@context` rather than a schema.
- [Data models](../../guides/data-models.md), targeting a Smart Data Model versus an invented one, and what each gives up.
