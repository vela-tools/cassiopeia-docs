...
sidebar_label: "JSON field mapping"
sidebar_position: 1
description: "Map a JSON array to ChemicalElement entities and type one copied field as a number."
keywords: ["NGSI-LD", "worked example", "json to NGSI-LD", "ChemicalElement"]
---

<!-- Synced from vela-tools/cassiopeia-examples. Edit the page there, not here. -->
# Mapping JSON fields into entities

A JSON array of chemical elements becomes NGSI-LD `ChemicalElement` entities. Because each symbol is unique, it can serve as the entity ID. Most attributes are copied unchanged; one transformation turns the atomic number from text into a number. The [next example](../02-json-id-collision/index.md) shows what happens when the obvious identity is not unique.
...
The dataset is not stored in this repository, and `data` is ignored by git. The runner downloads it from the same URL, which [example.toml](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/01-json-field-mapping/example.toml) records along with the publisher and the licence.
...
The command uses `--context none` so the example does not need a JSON-LD context or a Smart Data Models catalog. [Output](../../guides/output.md) explains how to attach a real context later.
