---
title: "Guarding GeoJSON attributes before validation"
sidebar_label: "Attribute guards"
sidebar_position: 5
description: "Combine conditional values with constants and omit invalid source values."
keywords: ["NGSI-LD", "worked example", "geojson to NGSI-LD", "OffStreetParking"]
---

<!-- Synced from vela-tools/cassiopeia-examples. Edit the page there, not here. -->
# Guarding GeoJSON attributes before validation

This builds on the [Smart Data Model example](../03-geojson-smart-data-model/index.md) by handling the two source fields that need conditional logic. The dataset and download steps stay the same. The mapping chooses vocabulary terms, combines a derived value with a constant in an array, and omits an attribute when its source value would fail validation.

## Get the data

This example reads the same [disabled-parking layer](https://www.data.gv.at/katalog/en/dataset/4315e096-f51e-4b56-8235-57be9789a62c) published by the City of Vienna as [example 3](../03-geojson-smart-data-model/index.md), and validates against the same Smart Data Models catalog. Download both into this example's `data` directory:
...
The full mapping is in [parking.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/05-geojson-attribute-guards/parking.json5). It starts from the Smart Data Model example, changes `category` to a two-entry array, and adds `requiredPermit` and `totalSpotNumber`:
