---
title: "Marking CSV rows as observations with observedAt"
sidebar_label: "observedAt observations"
sidebar_position: 7
description: "Keep monthly measurements under one entity ID with observedAt."
keywords: ["NGSI-LD", "worked example", "csv to NGSI-LD", "GoldPriceObserved"]
---

<!-- Synced from vela-tools/cassiopeia-examples. Edit the page there, not here. -->
# Marking CSV rows as observations with observedAt

In [example 2](../02-json-id-collision/index.md), records with the same ID were merged. A time series needs different handling. Every monthly gold price belongs to the same entity, but each row is a separate measurement in time.
...
The full mapping is in [gold.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/07-csv-observed-at/gold.json5). `GoldPriceObserved` is a plain type name, so this run does not validate against a schema.
