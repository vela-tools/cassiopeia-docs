---
title: "Creating synthetic JSON entities from a field"
sidebar_label: "Synthetic entities"
sidebar_position: 13
description: "Create country entities from a mountain's country field."
keywords: ["NGSI-LD", "worked example", "json to NGSI-LD", "Mountain", "Country"]
---
...
`Mountain` and `Country` are custom models with no published schemas. In `fail-when-schema` mode they are written without schema checks, just like the schemaless country in the [previous graph](../11-csv-relationship-graph/index.md).
