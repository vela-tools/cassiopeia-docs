---
title: "Mapping GeoJSON to a Smart Data Model"
sidebar_label: "GeoJSON Smart Data Model"
sidebar_position: 3
description: "Map GeoJSON to OffStreetParking and validate it against the published schema."
...
The full mapping is in [parking.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/03-geojson-smart-data-model/parking.json5). It maps the street and house number, description, opening hours, and coordinates directly, without conditional logic.
...
Two source fields are deliberately left for a [later example](../05-geojson-attribute-guards/index.md), which returns to this same parking dataset. They need conditional logic to pass validation: the numeric `KATEGORIE` must become the model's `category` and `requiredPermit` vocabulary values, while `STELLPL_ANZ` uses `-1` for an unknown count, which `totalSpotNumber` rejects.
