---
title: "Keeping variable GeoJSON objects in a JsonProperty"
sidebar_label: "JsonProperty"
sidebar_position: 25
description: "Keep an alert's changing parameters object in a JsonProperty."
keywords: ["NGSI-LD", "worked example", "geojson to NGSI-LD", "WeatherAlert"]
---
...
Like examples 24 and 26, this example uses a **custom** data model. See the [data-model guide](../../guides/data-models.md) for background.
...
- How to pass a GeoJSON feature's polygon through as a `GeoProperty`, as in the [parking example](../03-geojson-smart-data-model/index.md).
...
The complete mapping is in [alert.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/25-geojson-json-property/alert.json5). Scalar alert fields become ordinary Properties, the two timestamps use `datetime`, the feature polygon passes through as a `GeoProperty`, and `parameters` becomes the `JsonProperty`:
...
The polygon's positions are the source's, in reverse order. The alert feed winds its exterior ring clockwise; RFC 7946 clause 3.1.6 states the opposite as a producer requirement, so Cassiopeia rewinds every exterior ring it emits counterclockwise. See the [mapping guide](../../guides/mapping.md#geoproperty) for the geometry normalisation rules.

## When a JsonProperty, and when not

Use a `JsonProperty` when a value varies between records or is an opaque document that the consumer will parse. Prefer real attributes, a nested `Property` with [`mappings`](../03-geojson-smart-data-model/index.md), or top-level attributes when the structure is stable and should be queryable or validated. The inner keys of a `JsonProperty` are invisible to NGSI-LD queries and schemas. The [attribute-type reference](../../reference/ngsi-ld/attribute-types.md#jsonproperty) explains the distinction.
