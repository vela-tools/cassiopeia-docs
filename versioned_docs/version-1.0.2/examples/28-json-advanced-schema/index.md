...
sidebar_label: "Advanced schema validation"
sidebar_position: 28
description: "Validate wrappers and metadata in normalized output."
keywords: ["NGSI-LD", "worked example", "json to NGSI-LD", "FoodProduct"]
---
...
The [food-product.schema.json](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/28-json-advanced-schema/food-product.schema.json) schema uses draft-07 and describes each attribute's normalized wrapper. Only `id` and `type` are required. Measurements remain optional because a blank cell is omitted rather than emitted as null. The important constraints are ones a simplified schema cannot express:
...
The run is defined by the self-contained [manifest.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/28-json-advanced-schema/manifest.json5):
...
The [food-product.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/28-json-advanced-schema/food-product.json5) mapping carries no `schema` field. It only describes the record-to-entity transformation:
...
Rerun `cassiopeia map --manifest manifest.json5`. Products still emit `sugars` with `unitCode: "GRM"`, so validation fails. With `mode: "fail"`, the run stops before writing output. With `warn`, it completes and counts the mismatches. The [validation guide](../../guides/validation.md#validation-modes) describes the available outcomes.

## Related pages

- [A custom model validated against its own schema](../27-csv-custom-schema/index.md), the simplified-form counterpart: a custom schema over the key-values shape.
- [A multilingual name](../08-json-language-property/index.md), the LanguageProperty in depth.
- [A controlled-vocabulary term as an IRI](../26-csv-vocab-property/index.md), the VocabProperty in depth.
- [An ordered list of values](../24-json-list-property/index.md), the ListProperty in depth.
- [A variable object kept whole](../25-geojson-json-property/index.md), another custom model built around one attribute kind.
