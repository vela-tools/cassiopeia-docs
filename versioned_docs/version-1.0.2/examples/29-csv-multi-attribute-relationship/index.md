---
title: "Mapping CSV multi-attribute relationships across airport roles"
sidebar_label: "Multi-attribute relationships"
sidebar_position: 29
description: "Give a Flight one servesAirport name for its departure and arrival airports, distinguished by datasetId."
keywords: ["NGSI-LD", "worked example", "csv to NGSI-LD", "Flight", "Airport", "Airline", "AircraftModel", "Country"]
---
...
The [relationship graph example](../11-csv-relationship-graph/index.md) gives a `Flight` two airport attributes: `departsFromAirport` and `arrivesToAirport`. Both link a flight to an airport, but they describe different roles. This example puts those links under one `servesAirport` attribute and uses `datasetId` to keep the departure and arrival instances separate.
...
A `ListRelationship` stores several targets as one ordered `objectList`. It fits a variable-length collection of equivalent links, such as the aircraft codes listed on a route. The [list relationship example](../12-csv-list-relationship/index.md) shows that form.
...
The [datasetId example](../21-json-dataset-id/index.md) uses the same mapping pattern for several forecast values under one Property name. Here, the pattern separates two roles of one Relationship.
...
The cached `dataModel.Aeronautics/Flight` schema requires `id` and `type`, but it does not set `additionalProperties: false`. It therefore accepts the new `servesAirport` attribute because the mapping adds a new attribute rather than changing one defined by the model. This differs from the [ListRelationship example](../12-csv-list-relationship/index.md), where changing `hasAircraftModel` from a Relationship to a ListRelationship no longer matches the published schema.
