---
title: "Mapping XML unit codes and observed timestamps"
sidebar_label: "XML unit codes"
sidebar_position: 14
description: "Map XML measurements with unitCode and observedAt."
keywords: ["NGSI-LD", "worked example", "xml to NGSI-LD", "WeatherObserved"]
---
...
An `observedAt` makes the entity temporal. The station ID stays stable, while the timestamp keeps successive readings distinct. The [gold-price example](../07-csv-observed-at/index.md) uses the same approach for a monthly series; here each fetch adds a new observation for the station. The model also requires `dateObserved`, which comes from the same instant.
