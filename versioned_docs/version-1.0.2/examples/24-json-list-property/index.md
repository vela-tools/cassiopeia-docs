...
sidebar_label: "ListProperty"
sidebar_position: 24
description: "Store hourly counts in an ordered ListProperty."
keywords: ["NGSI-LD", "worked example", "json to NGSI-LD", "BicycleCounter"]
---
...
It uses a **custom** data model because no published Smart Data Model represents an hourly counter in this shape. The [data-model guide](../../guides/data-models.md) explains when a custom model is appropriate.
...
- How preprocessing can turn a one-row-per-hour feed into one record per day with an ordered array, as in examples [20](../21-json-dataset-id/index.md) and [21](../22-csv-broker-temporal/index.md).
...
The complete mapping is in [counter.json5](https://github.com/vela-tools/cassiopeia-examples/blob/main/examples/24-json-list-property/counter.json5):
...
Use a `ListProperty` when values form an ordered sequence read by position. Use a plain `Property` with an array when order has no meaning, and a [`ListRelationship`](../12-csv-list-relationship/index.md) when the elements link to other entities. The [attribute-type reference](../../reference/ngsi-ld/attribute-types.md#listproperty) compares the three.
