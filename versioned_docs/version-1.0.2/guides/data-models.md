...
- **A `@context`**: the JSON-LD dictionary that maps each short term (`type`, `WeatherObserved`, `temperature`) onto a full IRI, so that two systems agree on what each term means. The [`@context` example](../examples/20-csv-at-context/index.md) shows one authored from scratch.
...
Several examples target published models: [`OffStreetParking`](../examples/03-geojson-smart-data-model/index.md), [`WeatherObserved`](../examples/14-xml-unit-code/index.md), and the [airport/airline graph](../examples/11-csv-relationship-graph/index.md) all validate against the schemas the catalog publishes.
...
The [custom-`@context` example](../examples/20-csv-at-context/index.md) shows the process. The NASA Exoplanet Archive has no Smart Data Model, so the example defines an `ExoPlanet` type, writes a JSON-LD `@context` binding each term to an IRI, and attaches it with `--context local`. All three [attribute-type examples](../examples/index.md), [`BicycleCounter`](../examples/24-json-list-property/index.md), [`WeatherAlert`](../examples/25-geojson-json-property/index.md), and [`UrbanMobilityPoint`](../examples/26-csv-vocab-property/index.md), also use custom models. Each isolates one NGSI-LD attribute type, and a custom model keeps the focus on that type without incidental requirements from a published schema.

A custom model does not include the catalog's ready-made schema, shared vocabulary, or published `@context`, but you keep full control and can still use validation. The attributes are exactly the ones the data supports, named as the data reads, in the shapes the data has. Point Cassiopeia at a JSON Schema you author and it enforces your custom model just as it enforces a catalog model, checking either the simplified surface or the full normalized shape. [Example 27](../examples/27-csv-custom-schema/index.md) validates a custom model against a hand-written key-values schema, and [example 28](../examples/28-json-advanced-schema/index.md) validates one against a normalized schema that checks wrappers and metadata. The trade-off is the missing ready-made material, not reduced capability.
